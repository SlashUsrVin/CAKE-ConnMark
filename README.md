...
| __MATCHBY__ [_PORT or IP_] | Determines the matching criteria for (mangle) rule creation (NOT conntrack). Use PORT to match by port only, IP to match by IP address only, or BOTH to match by both IP and port. If not specified, BOTH is used by default.  | MATCHBY PORT |  
...
#### Example 1: MATCHBY is BOTH (or omitted)
    -A FORWARD -s 192.168.2.10/32 -j GAMING
    -A FORWARD -d 192.168.2.10/32 -j GAMING
    -A FORWARD -d 192.168.2.199/32 -j STREAMING
    -A GAMING -s 1.1.1.1/32 -p tcp -m tcp --sport 7300 -j DSCP --set-dscp 0x2e
    -A GAMING -d 1.1.1.1/32 -p tcp -m tcp --dport 7300 -j DSCP --set-dscp 0x2e
    -A GAMING -j RETURN
    -A STREAMING -s 8.8.8.8/32 -p tcp -m tcp --sport 443 -j DSCP --set-dscp 0x28
    -A STREAMING -j RETURN  

#### Example 2: MATCHBY is PORT (see GAMING lines) (match only by port)
    -A FORWARD -s 192.168.2.10/32 -j GAMING
    -A FORWARD -d 192.168.2.10/32 -j GAMING
    -A FORWARD -d 192.168.2.199/32 -j STREAMING
    -A GAMING -p tcp -m tcp --sport 7300 -j DSCP --set-dscp 0x2e
    -A GAMING -p tcp -m tcp --dport 7300 -j DSCP --set-dscp 0x2e
    -A GAMING -j RETURN
    -A STREAMING -s 8.8.8.8/32 -p tcp -m tcp --sport 443 -j DSCP --set-dscp 0x28
    -A STREAMING -j RETURN   